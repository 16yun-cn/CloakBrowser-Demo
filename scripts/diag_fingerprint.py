...
def _build_args(overrides=None):
    ...
    args = [
        ...
        f"--fingerprint-brand-version={fp['brand_version']}",
        f"--fingerprint-platform-version={fp['platform_version']}",
        f"--fingerprint-gpu-vendor={fp['gpu_vendor']}",
        f"--fingerprint-gpu-renderer={fp['gpu_renderer']}",
        f"--fingerprint-hardware-concurrency={fp['hardware_concurrency']}",
        f"--fingerprint-device-memory={fp['device_memory']}",
        f"--fingerprint-screen-width={fp['screen_width']}",
        f"--fingerprint-screen-height={fp['screen_height']}",
        f"--fingerprint-timezone={fp['timezone']}",
        f"--fingerprint-locale={fp['locale']}",
        f"--fingerprint-storage-quota={fp['storage_quota_mb']}",
        f"--fingerprint-webrtc-ip={fp['webrtc_ip_mode']}",
    ]
...
def install_bypass(page):
    def _fetch(route):
        try:
            ...
            rh.pop("proxy-authorization", None)
            rh.pop("proxy-connection", None)
            req = _urlreq.Request(route.request.url, headers=rh)
            with _urlreq.urlopen(req, timeout=15) as r:
                body = r.read()
                hd = dict(r.headers)
                hd.pop("transfer-encoding", None)
                hd.pop("content-encoding", None)
...
def fill_input(page, loc, text):
    ...
    if tag in ("textarea", "input"):
        ...
        loc.evaluate(
            "(el,t)=>{el.focus();el.textContent=t;el.dispatchEvent(new InputEvent('input',{bubbles:true,inputType:'insertText',data:t}))}",
            text,
        )
...
def trigger(page, prompt="你好，请介绍一下你自己"):
    ...
    if not el:
        log("WARN: no input found")
        return False
    fill_input(page, el, prompt)
    time.sleep(0.5)
    log("Clicking submit...")
    click_submit(page, el)
    return True
...
def signals(page):
    return page.evaluate(
        """()=>({wd:navigator.webdriver,ua:navigator.userAgent,pl:navigator.platform,hc:navigator.hardwareConcurrency,dm:navigator.deviceMemory,lg:navigator.languages,plugs:Array.from(navigator.plugins).map(p=>p.name),wc:typeof window.chrome,wcr:!!(window.chrome&&window.chrome.runtime),sc:{w:screen.width,h:screen.height},in:{w:window.innerWidth,h:window.innerHeight},tz:Intl.DateTimeFormat().resolvedOptions().timeZone})"""
    )


def canvas_hash(page):
    return page.evaluate(
        """()=>{const c=document.createElement('canvas');c.width=280;c.height=60;const x=c.getContext('2d');x.textBaseline='top';x.font='14px Arial';x.fillStyle='#f60';x.fillRect(125,1,62,20);x.fillStyle='#069';x.fillText('Test!',2,15);const d=c.toDataURL();let h=0;for(let i=0;i<d.length;i++){h=((h<<5)-h)+d.charCodeAt(i);h|=0;}return{len:d.length,hash:h}}"""
    )
...
def t_noise():
    ...
    try:
        ...
        errs = [
            m
            for m in logs
            if any(k in m.lower() for k in ["captcha", "verify", "fingerprint", "fail", "error", "5202"])
        ]
        if errs:
            print(f"\n  Console ({len(errs)}):")
            for m in errs[-15:]:
...
def t_windows():
    ...
    try:
        ...
        try:
            ...
            time.sleep(2)
        s = signals(page)
        log(f"platform={s['pl']} webdriver={s['wd']}")
        trigger(page)
...
def t_stock():
    ...
    try:
        ...
        ctx = pw.chromium.launch_persistent_context(
            str(DIAG_DIR / "stock"), headless=False, viewport={"width": 1440, "height": 900}
        )
        page = ctx.new_page()
        page.goto("https://www.doubao.com", wait_until="domcontentloaded")
        try:
        ...
        log("SKIP: playwright not installed")
    finally:
        try:
            ctx.close()
            pw.stop()
    ...
    try:
        ...
        for i in range(25):
            ...
            if c["found"]:
                ...
                im = d.get("imgs", [])
                ld = sum(1 for x in im if x.get("nw", 0) > 20)
                log(f"t={i + 1}s: imgs={len(im)} loaded={ld} err={d.get('err')}")
            time.sleep(1)
...
if __name__ == "__main__":
    ...
    if cmd == "all":
        ...
        print(f"Unknown: {cmd}. Options: {', '.join(STEPS.keys())}")
